	    ...
	    if (isset($_GET['st'])) {
	      if ($_GET['st']==1) {
	        ...
	        if ($conn->query("SELECT*FROM catatan WHERE id_user='$id_user'")->num_rows!==0) {
	            ...
	            echo "<div class='alert alert-warning'><strong>Tidak ada kritik dan saran yang diterima.</strong></div>";
	        }
    }elseif (isset($_SESSION['sw'])) {
        echo "<h3 class='sub-header'>Kritik dan Saran <button type='button' onclick=\"window.location.href='tambah_catatan';\" class='btn btn-success'>Tambah</button> </h3>";
